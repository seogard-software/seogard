...
export default defineEventHandler(async (event) => {
  ...
  const [pageAgg, alertAgg] = await Promise.all([
    MonitoredPage.aggregate([
      { $match: { siteId: siteObjectId, ...pathnameMatch } },
      // Compute pathname from url if missing
      {
        $addFields: {
          _pathname: {
            $ifNull: [
      ...
      // Filter: segments must start with drill path segments
      ...(drillSegments.length > 0
        ...
        : []),
      // Compute the child segment at drill depth
      {
        $addFields: {
          _childSegment: {
            $cond: {
    ...
    // 2. Aggregate alerts grouped by child segment (in parallel)
    Alert.aggregate([
      { $match: { siteId: site._id, status: 'open' } },
      // Extract pathname from pageUrl
      {
        $addFields: {
          _alertPathname: {
            $let: {
  ...
  log.info({ siteId: id, drill, pageGroups: pageAgg.length, alertGroups: alertAgg.length, queryMs }, 'MongoDB aggregation done')
