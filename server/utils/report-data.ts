...
export async function loadZoneCrawlHistory(siteId: string, zoneId: string): Promise<ZoneCrawlHistory> {
  ...
  const zoneName = zone.isDefault ? 'Toutes les pages' : (zone.name ?? 'Zone')
